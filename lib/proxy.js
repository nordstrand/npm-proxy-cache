...
exports.powerup = function(opts) {
    ...
    pem.createCertificate({
        ...
        https.createServer({key: keys.clientKey, cert: keys.certificate}, this.handler)
    ...
    //
    //https.createServer(options, this.handler).listen(opts.httpsport, opts.host, function(err) {
    //    if (err) throw err;
    //    exports.log.info('Proxying https %s:%s => %s [%d]', opts.host, opts.httpsport, process.env.TARGET_HOSTNAME, process.pid);
    //});
...
function respondWithCache(dest, cache, res) {
    ...
    //res.setHeader('Content-Length', cacheEntry.data.length);
